...
use crate::{
    dto::user_dto::{CreateUser, LoginDto},
    entities::user::{self, Column},
    error::AppError,
    state::AppState,
    utils::{
        jwt::generate_tokken,
        password_util::{encrypt_password, verify_password},
    },
};
...
async fn users(State(state): State<AppState>) -> Result<Json<Vec<user::Model>>, AppError> {
